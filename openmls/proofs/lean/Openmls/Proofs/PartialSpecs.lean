...
--
-- FOLLOW-UP: `level` is deliberately absent — it is a loop, so a partial contract for it needs a
-- partial loop lemma (a `⇓?`-flavoured variant of `level_loop_spec` in `Proofs.lean`) rather than
-- the straight-line reasoning used below. Tracked separately.
import Aeneas
import CoreModels
