...
-- is a *real definition*, not an opaque. The proved counterparts of such specs live in
-- `MissingCoreSpecs.lean`; the mathematical content that consumes these contracts is proved in
-- `Proofs.lean`. Hand-written; not regenerated by extraction.
import Aeneas
import CoreModels
...
/-- `<Vec<T> as DerefMut>::deref_mut` never fails and returns the slice view together with a
    ...
    Registered as `@[spec]`, so `mvcgen` picks it up automatically; no proof lists it explicitly.
    ...
    `match` on it unreduced — independent of this postcondition's shape. Its only consumer,
    `common_direct_path.spec.proof` in `Proofs.lean`, is unaffected: that spec's postcondition is
    `True` and the residual side goals close by `simp`/`omega`. -/
@[spec]
