...
- Function contracts (pre/post-conditions) live directly in the Rust source
  ...
  extraction.
- All 19 proof obligations — panic-freedom plus the functional postconditions — are
  proved in [`lean/`](lean/). The trusted surface is empty: no admitted contracts, no
  axioms beyond Lean's three standard ones (`propext`, `Classical.choice`, `Quot.sound`);
  the correspondence between the proved theorems and the generated specifications is
  machine-checked (`lean/Openmls/Proofs/Verification.lean`).
